      ...
      <div class="card">
        ...
        <div class="card-body">
          ...
          <form action="{{route('admin.order.charge', $order->id)}}"  method="post">
            ...
            <a href="{{route('admin.order.invoice',$order->id)}}" class="ml-2 btn btn-info">
            Generate Invoice</a>

          </form>
          <hr>

          <form action="{{route('admin.order.completed', $order->id)}}"  class="form-inline" style="display:inline!important;" method="post" >
            {{@csrf_field()}}
          ...
          <form action="{{route('admin.order.paid', $order->id)}}" class="form-inline" style="display:inline!important;" method="post">
            {{@csrf_field()}}
            @if($order->is_paid)
